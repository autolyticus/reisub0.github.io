...
Here it is again, with the maze context[ref][Source](https://coderscat.com/breadth-first-searchbfs-explained-with-visualization)[/ref]:
...
In both of the following examples which are real-world scenarios, both of the above strategies are sub-optimal. You will have to use a combination of them, carefully using your judgment to pick which topics are worth exploring deeper, and which are only worth skimming through (for now).
